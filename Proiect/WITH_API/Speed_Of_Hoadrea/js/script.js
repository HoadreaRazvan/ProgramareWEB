...
const carBrands = ["Ferrari", "Lamborghini", "Porsche", "BMW", "Mercedes", "Audi", "Tesla"];

const getRandomCarBrand = () => {
  const randomIndex = Math.floor(Math.random() * carBrands.length);
  return carBrands[randomIndex];
}

const getCarImageUrl = async (brand) => {
  const response = await fetch(`https://carimagery.com/api.asmx/GetImageUrl?searchTerm=${brand}`);
  const text = await response.text();
  const parser = new DOMParser();
  const xml = parser.parseFromString(text, "text/xml");
  const imageUrl = xml.getElementsByTagName("string")[0].textContent;
  return imageUrl;
}
...
async function startGame(event) {
  ...
  for (let i = 1; i < getQuantityElements(HEIGHT_ELEM * setting.traffic); i++) {
    ...
    const carBrand = getRandomCarBrand();
    const enemyImageUrl = await getCarImageUrl(carBrand);
    enemy.style.background = `transparent url(${enemyImageUrl}) center / cover no-repeat`;

    gameArea.appendChild(enemy);
  }

  // Set player car image
  const playerCarBrand = getRandomCarBrand();
  const playerCarImageUrl = await getCarImageUrl(playerCarBrand);
  car.style.background = `transparent url(${playerCarImageUrl}) center / cover no-repeat`;
...
function moveEnemy() {
  ...
  enemies.forEach(async function(enemy) {
